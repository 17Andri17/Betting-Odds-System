...
def generate_html_match_list(df):
    ...
    match_template = """
    <a href="/Statystyki_Serie_A?home_team={home_team}&date={original_date}" target=_self>
    <div class="match">
...
with col1:
    season_filter = st.multiselect("Wybierz sezon, z którego chcesz zobaczyć tabelę oraz statystyki",
        options = standings['season'].unique(), on_change=showDateButton, max_selections=1, key="SAseason_filter")
...
with filtr1:
    team_filter = st.multiselect("Wybierz drużynę", options = sorted(df_filtered['home_team'].unique()), key="SAteam_filter")
# with filtr2:
