...
if st.session_state.get("dfLL", None) is None:
    ...
    def showStats(i):
        st.session_state[f"LLshow_row_{i}"] = not st.session_state.get(f"LLshow_row_{i}", False)

    # chyba do usunięcia
    def showDateButton():
        if len(season_filter_matches) == 0:
            st.session_state["show_tableLL"] = True
        else:
            st.session_state["show_tableLL"] = False
        restartStats()
    ...
    df = st.session_state["dfLL"].copy()
    standings = st.session_state["standingsLL"].copy()
    df_filtered=df.copy()
    standings_filtered=standings.copy()

    if "LLseason_filter" not in st.session_state:
        st.session_state["LLseason_filter"] = []
    if "LLteam_filter" not in st.session_state:
        st.session_state["LLteam_filter"] = []
    if "LLnumber_of_matches" not in st.session_state:
        st.session_state["LLnumber_of_matches"] = 10
    ...
    with col1:
        season_filter = st.multiselect("Wybierz sezon, z którego chcesz zobaczyć tabelę oraz statystyki",
            options = standings['season'].unique(), on_change=showDateButton, max_selections=1, default=st.session_state["LLseason_filter"], key="LLseason_filter")
    ...
    st.subheader(f"Tabela La Ligi w sezonie {season_filter_matches}")
    ...
    with filtr1:
        team_filter = st.multiselect("Wybierz drużynę", options = sorted(df_filtered['home_team'].unique()), default=st.session_state["LLteam_filter"], key="LLteam_filter")
    with filtr2:
        number_of_matches = st.slider("Wybierz liczbę wyświetlanych meczów", min_value=10, max_value=100, step=5, value=st.session_state["LLnumber_of_matches"], key="LLnumber_of_matches")
    ...
    for i in range(min(number_of_matches, df_filtered['home_team'].count())):
        ...
        if st.session_state.get(f"LLshow_row_{i}", False):
            ...
            if st.button(
                ...
                key=f"LLshow_stats_button_{i}",
                args=(i,),
            ):
                restartStats()
                st.session_state["LLstats_id"] = df_filtered.iloc[i]
