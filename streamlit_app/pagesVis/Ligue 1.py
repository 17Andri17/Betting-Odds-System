...
if st.session_state.get("dfL1", None) is None:
    st.write("Odwiedź najpierw stronę główną aby załadować dane")
else:
    # Chowanie statystyk po zmianie filtrów
    def restartStats():
        for i in range (st.session_state["L1number_of_matches"]):
            if f"L1show_row_{i}" in st.session_state:
                st.session_state[f"L1show_row_{i}"] = False

    # Pokazywanie statystyk dla i-tego meczu
    def showStats(i):
        st.session_state[f"L1show_row_{i}"] = not st.session_state.get(f"L1show_row_{i}", False)

    # chyba do usunięcia
    def showDateButton():
        if len(season_filter_matches) == 0:
            st.session_state["show_tableL1"] = True
        else:
            st.session_state["show_tableL1"] = False
        restartStats()
    ...
    df = st.session_state["dfL1"].copy()
    standings = st.session_state["standingsL1"].copy()
    df_filtered=df.copy()
    standings_filtered=standings.copy()

    if "L1season_filter" not in st.session_state:
        st.session_state["L1season_filter"] = []
    if "L1team_filter" not in st.session_state:
        st.session_state["L1team_filter"] = []
    if "L1number_of_matches" not in st.session_state:
        st.session_state["L1number_of_matches"] = 10
    ...
    with col1:
        season_filter = st.multiselect("Wybierz sezon, z którego chcesz zobaczyć tabelę oraz statystyki",
            options = standings['season'].unique(), on_change=showDateButton, max_selections=1, default=st.session_state["L1season_filter"], key="L1season_filter")
    ...
    st.subheader(f"Tabela Ligue 1 w sezonie {season_filter_matches}")
    ...
    with filtr1:
        team_filter = st.multiselect("Wybierz drużynę", options = sorted(df_filtered['home_team'].unique()), default=st.session_state["L1team_filter"], key="L1team_filter")
    with filtr2:
        number_of_matches = st.slider("Wybierz liczbę wyświetlanych meczów", min_value=10, max_value=100, step=5, value=st.session_state["L1number_of_matches"], key="L1number_of_matches")
    ...
    for i in range(min(number_of_matches, df_filtered['home_team'].count())):
        ...
        if st.session_state.get(f"L1show_row_{i}", False):
            ...
            if st.button(
                ...
                key=f"L1show_stats_button_{i}",
                args=(i,),
            ):
                restartStats()
                st.session_state["L1stats_id"] = df_filtered.iloc[i]
