...
def restartStats():
    for i in range (st.session_state["BLnumber_of_matches"]):
        if f"BLshow_row_{i}" in st.session_state:
            st.session_state[f"BLshow_row_{i}"] = False

# Pokazywanie statystyk dla i-tego meczu
def showStats(i):
    st.session_state[f"BLshow_row_{i}"] = not st.session_state.get(f"BLshow_row_{i}", False)

# chyba do usunięcia
def showDateButton():
    if len(season_filter_matches) == 0:
        st.session_state["show_tableBL"] = True
    else:
        st.session_state["show_tableBL"] = False
    #restartStats()
...
def generate_html_match_list(df):
    ...
    match_template = """
    <a href="/Statystyki_Bundesliga?home_team={home_team}&date={original_date}" target=_self>
    <div class="match">
...
with col1:
    season_filter = st.multiselect("Wybierz sezon, z którego chcesz zobaczyć tabelę oraz statystyki",
        options = standings['season'].unique(), on_change=showDateButton, max_selections=1, key="BLseason_filter")
...
with filtr1:
    team_filter = st.multiselect("Wybierz drużynę", options = sorted(df_filtered['home_team'].unique()), key="BLteam_filter")
# with filtr2:
